...
CREATE PROCEDURE dbo.pr_dashboard_purchase_boleto (@id_evento INT
        ,@id_apresentacao INT
        ,@date DATETIME
        ,@hour VARCHAR(5)
...
SELECT 
    @awaiting_payment = (SELECT total FROM #result WHERE in_situacao='P')
    ,@expired_payment = (SELECT total FROM #result WHERE in_situacao='E')
    ,@ok_payment = (SELECT total FROM #result WHERE in_situacao='F')
