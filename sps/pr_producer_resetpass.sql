...
CREATE PROCEDURE dbo.pr_producer_resetpass (@api VARCHAR(100), @id UNIQUEIDENTIFIER, @newPass VARCHAR(1000))

AS 

SET NOCOUNT ON;

UPDATE CI_MIDDLEWAY..producer SET [password]=@newPass, updated=GETDATE() WHERE id=@id
