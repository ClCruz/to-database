ALTER PROCEDURE dbo.pr_dashboard_purchase_paymenttype (@id_evento INT
        ,@id_apresentacao INT
        ,@date DATETIME
        ,@hour VARCHAR(5)
...
--         ,@type VARCHAR(10) = 'all' --- all, web, boxoffice
...
--         ,@periodInit DATETIME = NULL
--         ,@periodEnd DATETIME = NULL
--         ,@type VARCHAR(10) = 'boxoffice' --- all, web, boxoffice
...
CREATE TABLE #resultFinal ([type] VARCHAR(1000), [name] VARCHAR(1000), sold INT)
...
SET @toExec=''
SET @toExec = @toExec + 'INSERT INTO #resultAux (Indice,CodTipBilhete,CodSala,CodSetor,NomSala,NomSetor,TipBilhete,CodTipLancamento,CodForPagto,TipForPagto,DatVenda,ValPagto,inprocess,isok, web) '
...
SET @toExec = @toExec + ' ,l.CodForPagto '
...
-- AND (@type = 'all' AND (ra.web=1 OR ra.web=0)
--     OR
--     (@type = 'web' AND (ra.web=1))
--     OR
--     (@type = 'boxoffice' AND (ra.web=0))
-- )
-- -- AND (@type = 'boxoffice' OR ra.web=0)
...
INSERT INTO #resultFinal ([type], [name], sold)
SELECT
'all'
,ra.TipForPagto [name]
,COUNT(*) sold
