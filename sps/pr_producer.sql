...
WHERE 
((@search IS NULL OR p.ds_razao_social LIKE '%'+@search+'%' COLLATE SQL_Latin1_General_Cp1251_CI_AS)
OR (@search IS NULL OR p.cd_email LIKE '%'+@search+'%' COLLATE SQL_Latin1_General_Cp1251_CI_AS)
OR (@search IS NULL OR p.cd_cpf_cnpj LIKE '%'+@search+'%' COLLATE SQL_Latin1_General_Cp1251_CI_AS))
